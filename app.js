...
const POSITIONS = {
    ...
    basketball: {
        PG: { label: 'Point Guard', group: 'guard', attrs: ['Ball Handling','Passing','Three Point','Speed','Perimeter Defense','Vision','Finishing','Mid Range','Rebounding','Post Scoring','Post Defense','Shot Blocking','Strength','Athleticism'], h: 74, w: 180 },
        SG: { label: 'Shooting Guard', group: 'guard', attrs: ['Three Point','Mid Range','Finishing','Speed','Perimeter Defense','Ball Handling','Passing','Vision','Rebounding','Post Scoring','Post Defense','Shot Blocking','Strength','Athleticism'], h: 77, w: 195 },
        SF: { label: 'Small Forward', group: 'wing', attrs: ['Three Point','Finishing','Athleticism','Perimeter Defense','Rebounding','Ball Handling','Passing','Speed','Vision','Mid Range','Post Scoring','Post Defense','Shot Blocking','Strength'], h: 79, w: 215 },
        PF: { label: 'Power Forward', group: 'big', attrs: ['Post Scoring','Rebounding','Mid Range','Strength','Post Defense','Athleticism','Three Point','Finishing','Shot Blocking','Ball Handling','Passing','Speed','Perimeter Defense','Vision'], h: 81, w: 235 },
        C:  { label: 'Center', group: 'big', attrs: ['Post Scoring','Rebounding','Post Defense','Shot Blocking','Strength','Finishing','Three Point','Mid Range','Athleticism','Ball Handling','Passing','Speed','Perimeter Defense','Vision'], h: 83, w: 250 },
    },
    baseball: {
...
const AGE_MILESTONES = {
    ...
    basketball: { hsComplete: 18, collegeComplete: 19, proStart: 19 },
    baseball:   { hsComplete: 18, collegeComplete: 20, proStart: 21 },
...
function getCareer(sport, id) {
    const career = STATE.careers[sport].find(c => c.id === id);
    if (!career) return career;
    // Patch missing basketball attributes for old saves
    if (career.sport === 'basketball' && career.position) {
        const allAttrs = roleAttrs('basketball', career.position);
        allAttrs.forEach(a => {
            if (career.attributes[a] === undefined) {
...
function generateStartingAttributes(sport, posKey, heightIn, weightLb) {
    ...
    const primaryCount = (sport === 'basketball') ? 6 : attrs.length;
...
function canUpgrade(career, attrName) {
    if (isInDecline(career)) return false;
    const val = career.attributes[attrName];
    if (val >= ATTR_MAX) return false;
    if (career.skillPoints < upgradeCost(val)) return false;
    // Tier-based overall cap: can't push the overall above the tier ceiling
    const cap = TIER_OVERALL_CAPS[career.tier || DEFAULT_TIER] ?? 99;
    if (career.overall >= cap) return false;
    return true;
}
...
function updateCareerHighs(career, line) {
    const sport = career.sport;
    if (!career.careerHighs) career.careerHighs = {};
    const ch = career.careerHighs;
    const fields = statFieldsFor(sport, career.position || 'GEN');
    // Golf: lower is better for stroke categories
    const lowerIsBetter = new Set(['strokesTotal', 'relToPar']);
    fields.forEach(([k]) => {
        const val = line[k];
        if (val === undefined || val === null) return;
        if (lowerIsBetter.has(k)) {
            if (ch[k] === undefined || val < ch[k]) ch[k] = val;
        } else {
            if (ch[k] === undefined || val > ch[k]) ch[k] = val;
        }
    });
...
function generateFootballLine(posKey, perf, career) {
    ...
    switch (group) {
        case 'passer': {
            ...
            const passTD = p > 0.82 ? r(2, 3) + (Math.random() < 0.25 ? 1 : 0)
                : p > 0.72 ? r(1, 2) + (Math.random() < 0.20 ? 1 : 0)
                    : p > 0.60 ? r(1, 2)
                        : r(0, 1);
    ...
    if (REAL_LEAGUES[sport]) {
        teams = REAL_LEAGUES[sport].map((t, i) => {
            return {
                ...
                conference: t.conf,
                division: t.div,
                rating: clamp(Math.round(randNormal(74, 9)), 55, 97),
...
function buildSeasonSchedule(teamIds, totalGames) {
    ...
    for (let w = 0; w < totalGames; w++) {
        const pool = pickN(teamIds, teamIds.length);
        const used = new Set();
        const matchups = [];
...
function awardPoints(career, stage, perf, sport) {
    if (isFullyMaxed(career)) return 0;
...
function simulateHighSchoolGame(career) {
    ...
    else { line = genStatline(career, perf); addStatTotals(career.seasonStats, line); updateCareerHighs(career, line); summary = statlineSummary(career.sport, career.position, line); }
...
function declareTurnPro(career) {
    ...
    try {
        if (career.college && !career.careerStatsLog.some(s => s.label === career.college.name)) {
            archiveSeason(career, 'college', career.college.name);
        }
    ...
    career.tour = {
        ...
        schedule: generateTourSchedule(career.sport),
        eventIndex: 0,
    };
    ...
    career.season = 1;
    career.seasonGameCount = 0;
    career.seasonPerfSum = 0;
    career.seasonPerfGames = 0;
    ...
    if (playoffResult && playoffResult.madePlayoffs) {
        if (playoffResult.wonIt) {
            if (sport === 'football') {
                ...
                // Finals MVP: won the title — were you the best player?
                const bestOnTeam = wasBestOnOwnTeam(career);
...
function miniAvatar(career, size) {
    return `<div class="miniAvatar" style="width:${size}px;height:${size}px">${buildAvatarSVG({
        skinTone: career.appearance.skinTone,
        hairStyle: career.appearance.hairStyle,
        hairColor: career.appearance.hairColor,
        jerseyColor: SPORT_META[career.sport].accent,
...
function creatorFormHTML(sport) {
    ...
    <div class="creatorLayout">
      <div class="creatorPreview">
        ...
        skinTone: cs.appearance.skinTone,
        hairStyle: cs.appearance.hairStyle,
        hairColor: cs.appearance.hairColor,
        jerseyColor: sm.accent,
        number: cs.number,
        heightIn: cs.heightIn,
      ...
      <form class="creatorForm" id="creatorForm" novalidate>
        ...
        <section class="formSection">
          ...
          <div class="field">
            ...
            <div class="chipRow">
              ${HAIR_STYLES.map(s => `<button type="button" class="chip ${cs.appearance.hairStyle === s ? 'chip--active' : ''}" data-action="creator-pick-hairstyle" data-style="${s}">${hairStyleLabel(s)}</button>`).join('')}
            </div>
          </div>
          <div class="field">
            <span class="field__label">Hair Color</span>
            <div class="swatchRow">
              ${HAIR_COLORS.map(c => `<button type="button"
                  class="swatch swatch--sm ${activeColorName === c.name ? 'swatch--active' : ''}"
                  style="background:${c.hex}"
                  data-action="creator-pick-haircolor"
...
function refreshAvatarPreview(sport) {
    ...
    qs('avatarPreview').innerHTML = buildAvatarSVG({
        ...
        hairStyle: cs.appearance.hairStyle,
        hairColor: cs.appearance.hairColor,
        jerseyColor: SPORT_META[sport].accent,
        number: cs.number,
        heightIn: cs.heightIn,
        weightLb: cs.weightLb,
    });
...
function renderAttributesTab(career) {
    ...
    const tierCap = TIER_OVERALL_CAPS[career.tier || DEFAULT_TIER] ?? 99;
    ...
    ${declineBanner}
    <p class="formSection__hint">Spend skill points earned from simulating games to push individual attributes from 65 toward a max of 99. Costs rise as attributes get elite${rules ? `, and upgrades lock entirely once decline sets in around age ${rules.declineStart}` : ''}.${capNote}</p>
...
function renderBadgesSection(career) {
    ...
    const cards = list.map(b => {
        ...
        let btnHtml;
        if (has) { btnHtml = `<span class="badgeCard__owned">✓ Owned</span>`; }
        else {
    ...
    <section class="block" style="margin-top:24px">
      ...
      <p class="formSection__hint">A short list of late-career perks. Each one gives a small, permanent edge to performance — unlocked with skill points once your overall is high enough.</p>
...
function renderConditioningSection(career) {
    ...
    <section class="block" style="margin-top:24px">
      ...
      <p class="formSection__hint">Conditioning doesn't raise your overall — it slows the decline that sets in around age ${rules.declineStart} and pushes back retirement decline.</p>
      <div class="attrList">
        <div class="attrRow">
          <div class="attrRow__top">
...
function renderAwardsTab(career) {
    if (!career.proLeague || (career.stage !== 'pro' && career.stage !== 'retired')) {
      ...
      <p>Awards and award races show up here once you're in the pros.</p>
    </div>`;
    }
...
function renderLiveAwardRace(career, labels) {
    ...
    if (!keys.length) {
        return `<section class="block"><h2 class="block__title">This Season's Race</h2><p class="emptyHint">No award races apply to you this season.</p></section>`;
    }
...
function careerHighsHTML(career) {
    ...
    const sport = career.sport;
    const pos = career.position || 'GEN';
    const fields = statFieldsFor(sport, pos);
    const ch = career.careerHighs;
    const lowerIsBetter = new Set(['strokesTotal', 'relToPar']);
    const rows = fields
        .filter(([k]) => ch[k] !== undefined && k !== 'gp')
        .map(([k, label]) => {
            const note = lowerIsBetter.has(k) ? ' (lowest)' : '';
            return `<div class="gameLogRow">
        <div class="gameLogRow__main">
          <span class="gameLogRow__label">${label}${note}</span>
          <span class="gameLogRow__summary">${formatStatVal(k, ch[k])}</span>
        </div>
      </div>`;
        }).join('');
    return `<section class="block">
    <h2 class="block__title">Career Highs</h2>
      ...
      <form class="creatorForm" id="editForm">
        ...
        <section class="formSection">
          ...
          <div class="field"><span class="field__label">Hair Style</span><div class="chipRow">
            ${HAIR_STYLES.map(s => `<button type="button" class="chip ${career.appearance.hairStyle === s ? 'chip--active' : ''}" data-action="edit-pick-hairstyle" data-style="${s}">${hairStyleLabel(s)}</button>`).join('')}
          </div></div>
          <div class="field"><span class="field__label">Hair Color</span><div class="swatchRow">
            ${HAIR_COLORS.map(c => `<button type="button"
                class="swatch swatch--sm ${activeColorName === c.name ? 'swatch--active' : ''}"
                style="background:${c.hex}"
                data-action="edit-pick-haircolor"
                data-color-name="${c.name}"
                data-color-hex="${c.hex}"
                aria-label="${c.name} hair color"></button>`).join('')}
          </div></div>
...
function handleAction(action, btn) {
    ...
    switch (action) {
        ...
        case 'creator-pick-haircolor': {
            /* Store as a named object so saveState persists a stable value */
            creatorState.appearance.hairColor = { name: btn.dataset.colorName, hex: btn.dataset.colorHex };
            refreshCreatorForm(sport);
            return;
        }
        case 'creator-submit': {
        ...
        case 'edit-pick-skin': {
            ...
            c.appearance.skinTone = btn.dataset.tone;
            saveState();
            bindEditTabAfterRender(c);
            return;
        }
        case 'edit-pick-hairstyle': {
            const c = currentCareerFromRoute();
            c.appearance.hairStyle = btn.dataset.style;
            saveState();
            bindEditTabAfterRender(c);
            return;
        }
        case 'edit-pick-haircolor': {
            const c = currentCareerFromRoute();
            /* Store as object matching the creator format */
            c.appearance.hairColor = { name: btn.dataset.colorName, hex: btn.dataset.colorHex };
            saveState();
            bindEditTabAfterRender(c);
            return;
        }
        ...
        case 'retire-career': {
            const c = currentCareerFromRoute();
            // If already confirmed (second click), execute retirement
            if (btn.dataset.confirmed === 'true') {
                retireCareer(c);
                openModal(retirementModalHTML(c));
                return;
            }
            // First click — show inline confirmation
            openModal(`<div class="modal__head"><h2>Retire ${escapeHtml(c.name)}?</h2><button class="iconBtn" data-action="modal-close" aria-label="Close">✕</button></div>
    <div class="modal__body"><p>Are you sure you want to retire? This ends the career permanently. All stats are preserved in the Stats tab.</p></div>
    <div class="modal__foot">
      <button class="btn btn--ghost" data-action="modal-close">Cancel — Keep Playing</button>
      <button class="btn btn--danger" data-action="retire-career" data-confirmed="true">Yes, Retire Now</button>
    </div>`);
            return;
        }
        case 'extend-career': {
            ...
            extendCareer(c);
            rerenderCurrentRoute();
            toast('Career extended — keep going!', 'success');
            return;
        }
        case 'request-trade': {
            const c = currentCareerFromRoute();
