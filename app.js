...
const LEAGUE_SHAPE = {
  ...
  basketball:{ teams: 30, conferences: 2, divisions: 3, games: 82, directSeeds: 6, playInPool: 4, byeCount: 1 },
...
const RETIREMENT_RULES = {
  football:   { declineStart: 30, retireMin: 35, retireMax: 42 },
  basketball: { declineStart: 30, retireMin: 35, retireMax: 42 },
  baseball:   { declineStart: 30, retireMin: 35, retireMax: 42 },
  bowling:    { declineStart: 45, retireMin: 60, retireMax: 60 },
  golf:       { declineStart: 45, retireMin: 60, retireMax: 60 },
};
...
/* ----------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------- */
const PLAYOFF_ROUND_NAMES = ['Play-In', 'Wild Card Round', 'Divisional Round', 'Conference Championship', 'Championship'];
...
function buildAwardLeaderboard(career, key) {
  ...
  const entries = pool.map(t => {
    ...
    else grade = clamp(t.rating + randInt(-9, 9), 35, 99);
    return { name: fictionalStarName(t), teamName: t.name, isUser: false, grade };
  });
